<template>
  <div class="px-6 pt-6 pb-4">
    <h2 class="text-lg font-medium text-gray-900">{{ $t('components.rooms.title') }}</h2>
  </div>
  <nav class="flex-1 min-h-0 overflow-y-auto bg-white" :aria-label="$t('components.rooms.title')">
    <div v-for="floor in Object.keys(rooms)" :key="floor" class="relative">
      ...
      <ul class="relative z-0 divide-y divide-gray-200">
        <li v-for="room in rooms[floor]" :key="room.id" :class="room.id === $route.params.id ? 'bg-primary-50' : ''">
